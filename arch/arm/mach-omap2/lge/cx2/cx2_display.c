...
static struct lge_dsi_panel_data dsi2_lgd_panel = {
	...
	.use_ext_te		= true, //diff gb true <- false,
 	.ext_te_gpio		= GPIO_LCD_EXT_TE, //gb 103
	.esd_interval		= false,
	.set_backlight		= NULL
};
...
static struct omap_dss_device lgd_device = 
{
	.name	= "2lcd", //gb "2lcd"
#if defined(CONFIG_PANEL_LH430WV5_SD01) //##hwcho_20120514
	.driver_name	= "lh430wv5_panel", //gb "lh430wv5_panel"
#else
	.driver_name	= "lh430wv4_panel", //xxx
#endif //##	
	.type	= OMAP_DISPLAY_TYPE_DSI, // gb OMAP_DISPLAY_TYPE_DSI
	...
	.phy.dsi		= {
		.clk_lane	= 1, //gb 1
		.clk_pol	= 0, //gb 0(DSI_CLOCK_POLARITY)
		.data1_lane	= 3, //gb 3
		.data1_pol	= 0, //gb 0(DSI_DATA0_POLARITY)
		.data2_lane	= 2, //gb 2
		.data2_pol	= 0, //gb 0(DSI_DATA1_POLARITY)
	#if defined(CONFIG_DSI_VIDEO_MODE) || defined (CONFIG_DSI_VIDEO_CMD_TRANSITION)
		.type = OMAP_DSS_DSI_TYPE_VIDEO_MODE, //gb OMAP_DSI_MODE_VIDEO
	#else
		.type = OMAP_DSS_DSI_TYPE_CMD_MODE, //xxx
	#endif
		.module = DSS_IX_DSI2,
	},
	...
	.clocks = {
		.dispc = {
			.channel = {
				.lck_div	= 1,	 /* Logic Clock = 153.6 MHz */ //gb 1 .dsi.div.lck_div
				.pck_div	= 5,	 /* Pixel Clock = 30.7 MHz */ //gb 5 .dsi.div.pck_div
				.lcd_clk_src	= OMAP_DSS_CLK_SRC_DSI2_PLL_HSDIV_DISPC, //gb DSS_SRC_PLL2_CLK1 ? <- OMAP_DSS_CLK_SRC_DSI2_PLL_HSDIV_DISPC,
			},
			.dispc_fclk_src	= OMAP_DSS_CLK_SRC_FCK,
		},

		.dsi = {
			.regn		= 20,  /* Fint = 1.92 MHz */ //diff gb 20 <- 19,
			.regm		= 177,  /* CLKIN4DDR = 768.0 MHz */  //diff gb 177 <- 180,
			.regm_dispc	= 5,  /* PLL2_CLK1 = 153.6 MHz */ //diff gb 5 <- 4,
			.regm_dsi	= 7,  /* PLL2_CLK2 = 109.7 MHz */ //diff gb 7 <- 6,

			.lp_clk_div	= 12,	/* LP Clock */ //gb 12
			.dsi_fclk_src	= OMAP_DSS_CLK_SRC_DSI2_PLL_HSDIV_DSI, //diff gb DSS_SRC_PLL1_CLK2 ? <- OMAP_DSS_CLK_SRC_DSI2_PLL_HSDIV_DSI,
		},
	},

	.platform_enable	=	panel_enable_lcd, //gb cx2_panel_enable_lcd (GPIO_27)
	.platform_disable	=	panel_disable_lcd, //gb cx2_panel_disable_lcd (GPIO_27)
	.channel		= OMAP_DSS_CHANNEL_LCD2, //gb OMAP_DSS_CHANNEL_LCD2
...
lge_machine_initcall(p720_dss_init);
...
int __init p720_fb_init(void)
{
	return lge_set_fb_info(&fb_pdata);
}

lge_machine_initcall(p720_fb_init);

static void p720_dsi_phy_init(void)
{
	u32 reg;
...
static void p720_hdmi_pad_init(void)
{
	u32 r;
...
LGE_LATE_INIT(p720_dsi_phy_init);
LGE_LATE_INIT(p720_hdmi_pad_init);
