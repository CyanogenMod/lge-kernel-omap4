	...
	.clocks = {
		.dispc = {
			.channel = {
				...
				.pck_div	= 4, //5,	/* Pixel Clock = 34.56 MHz */
				.lcd_clk_src	= OMAP_DSS_CLK_SRC_DSI_PLL_HSDIV_DISPC,
			},
			.dispc_fclk_src	= OMAP_DSS_CLK_SRC_DSI_PLL_HSDIV_DISPC,//OMAP_DSS_CLK_SRC_FCK,
		},

		.dsi = {
			.regn		= 18, //16,	/* Fint = 2.4 MHz */
			.regm		= 200, //180,	/* DDR Clock = 216 MHz */
			.regm_dispc	= 3, //5,	/* PLL1_CLK1 = 172.8 MHz */
			.regm_dsi	= 6, //5,	/* PLL1_CLK2 = 172.8 MHz */

			.lp_clk_div	= 12, //10,	/* LP Clock = 8.64 MHz */
...
int __init lghdk_dss_init(void)
{
	return lge_set_dss_info(&dss_board_info);
}

lge_machine_initcall(lghdk_dss_init);

static void lghdk_lcd_dsi_phy_init(void)
{
	u32 reg;
...
static void lghdk_hdmi_pad_init(void)
{
	u32 r;
...
LGE_LATE_INIT(lghdk_lcd_dsi_phy_init);
LGE_LATE_INIT(lghdk_hdmi_pad_init);
