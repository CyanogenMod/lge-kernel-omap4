/* 
 * arch/arm/mach-omap2/lge/lge_handle_panic.c
 *
...
#define HIDDEN_REBOOT	0x729F2000
...
int lge_hidden_reset_mode = 0;
static int __init set_hidden_reset_mode(char *hreset_on)
{
	if( !strncmp(hreset_on, "on", 2) ) {
		lge_hidden_reset_mode = 1;
		printk("reboot mode : hidden reset %s\n", "on");
	}

	return 1;
}
__setup("lge.hreset=", set_hidden_reset_mode);
