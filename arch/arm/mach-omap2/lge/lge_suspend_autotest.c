...
enum android_alarm_type alarm_type = ANDROID_ALARM_RTC_WAKEUP;
static struct alarm alarm_dev;
#endif
...
#if USE_ANDROID_ALARM
static void set_alarm(unsigned long s)
{
	struct timespec rt;
	...
	alarm_start_range(&alarm_dev,
			timespec_to_ktime(rt),
			timespec_to_ktime(rt));
}
...
#if USE_ANDROID_ALARM
	set_alarm(now + TEST_SUSPEND_SECONDS);
#else
