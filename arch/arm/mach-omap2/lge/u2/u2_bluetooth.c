...
static int bcm_bt_lpm_init(struct platform_device *pdev)
{
	...
	rc = gpio_request(GPIO_BT_WAKE, "bcm4330_wake_gpio");
	if (unlikely(rc)) {
		return rc;
	}

	rc = gpio_request(GPIO_BT_HOST_WAKE, "bcm4330_host_wake_gpio");
	if (unlikely(rc)) {
...
static int btdebug_dump(struct seq_file *sf, void *private)
{
	seq_printf(sf, "en=%d bt_wake=%d lpm.w=%d w_uart_en=%d\n",
		   bt_enabled, gpio_get_value(GPIO_BT_WAKE),
		   bt_lpm.wake, wake_uart_enabled);
	seq_printf(sf, "bt_host_wake=%d lpm.hw=%d hw_uart_en=%d\n",
		   gpio_get_value(GPIO_BT_HOST_WAKE), bt_lpm.host_wake,
...
static int bcm4330_bluetooth_probe(struct platform_device *pdev)
{
	...
	rc = gpio_request(GPIO_BT_RESET, "bcm4330_nreset_gpio");
	if (unlikely(rc)) {
		return rc;
	}
