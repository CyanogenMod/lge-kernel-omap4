...
exit_work:
		...
		if (!ts->is_probed) {
...
#if defined(TEST_BOOTING_TIME_FW_FORCE_UPGRADE)
			{
				...
				printk("Synaptics %s(%s): Manufacturer ID=%d, FW revision=%d\n",
		...
		if (ret < 0)
	 	...
	 	printk("DEVICE_CONTROL_REG write success\n");
		...
		ret = synaptics_ts_read(ts->client, INTERRUPT_STATUS_REG, 1, &temp);
		if (ret < 0)
			printk("INTERRUPT_STATUS_REG read fail\n");
...
static int synaptics_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (likely(ts->pdata->use_irq && client->irq)) {
		...
		ret = request_threaded_irq(gpio_to_irq(client->irq), synaptics_ts_irq_handler,
		...
		if (ret < 0) {
			...
			printk("touch : request_irq failed. use polling mode\n");
		} else {
			if (synaptics_rmi4_i2c_debug_mask & SYNAPTICS_RMI4_I2C_DEBUG_FUNC_TRACE)
			{
	...
	queue_delayed_work(synaptics_wq, &ts->work, 0);
	printk("Synaptics Touch init success\n");
