 ...
 * so the dummy function is defined here to
 * avoid build error.
 * Plase replace this function to suitable one.
 */
   WARN_ON(1);
 }
#else
...
#ifdef CONFIG_MHL_LG_MOTION_CTL
static mhlTx_config_t_scratchpadData mhlTxConfig_scratchPadData;
...
void process_cbus_interrrupt(void)
{
    ...
    {
#ifdef CONFIG_MHL_LG_MOTION_CTL
		printk("sii9244_driver.c -> CONFIG_MHL_LG_MOTION_CTL process_cbus_interrupt, intr_cbus2 & INTR_CBUS2_DESIRED_MASK \n");
#endif    
		if(intr_cbus2& BIT0)
        {
...
void mhltx_got_mhlintr(u8 data0, u8 data1)
{
    ...
    {
#ifdef CONFIG_MHL_LG_MOTION_CTL
    	printk("sii9244_driver.c -> CONFIG_MHL_LG_MOTION_CTL mhltx_got_mhlintr \n");
#endif
...
void SiiMhlTxReadScratchpad(void)
{
	printk("sii9244_driver.c -> SiiMhlTxReadScratchpad \n");
	int m_idx = 0;

	memset(mhlTxConfig_scratchPadData.mscScratchpadData, 0x00, MHD_SCRATCHPAD_SIZE);

	for(m_idx=0; m_idx<(MHD_SCRATCHPAD_SIZE/2); m_idx++)
	{
		mhlTxConfig_scratchPadData.mscScratchpadData[m_idx] = ReadByteCBUS(0xC0+m_idx);
		printk("%s, [%d] %x\n", __func__, m_idx, mhlTxConfig_scratchPadData.mscScratchpadData[m_idx]);
	}
...
void SiiMhlTxWriteScratchpad(byte *wdata)
{
    ...
    cbus_cmd_bind(MHL_WRITE_BURST,0x40, u8* data, MHD_MAX_BUFFER_SIZE,FALSE);
}
*/

void MhlControl(void)
{
	printk("sii9244_driver.c -> MhlControl \n");
	byte category = 0;
	byte command = 0;
	byte action = 0;
	byte r_action = 0;
	...
	category = mhlTxConfig_scratchPadData.mscScratchpadData[0];

	if(category == 0x01)
	{
		printk("sii9244_driver.c -> category 0x01 \n");
		command =  mhlTxConfig_scratchPadData.mscScratchpadData[1];
		if(command == 0x01)			// Magic Motion Remote Controller -> absolute position, button
		{
		printk("sii9244_driver.c -> category 0x01 command 0x01 \n");
			action = mhlTxConfig_scratchPadData.mscScratchpadData[2];
			x_position = (int)((mhlTxConfig_scratchPadData.mscScratchpadData[3] <<8) | mhlTxConfig_scratchPadData.mscScratchpadData[4]);
			y_position = (int)((mhlTxConfig_scratchPadData.mscScratchpadData[5] <<8) | mhlTxConfig_scratchPadData.mscScratchpadData[6]);
			...
			#else
			{
				...
				sprintf(mbuf, "CONFIG_MHL_LG_MOTION_CTL action=%04d, x_pos=%04d, y_pos=%04d", r_action,x_position,y_position);
			       hdmi_common_send_uevent(x_position, y_position,r_action,tvCtl_x,tvCtl_y);
			}
			#endif
		}
		else if(command == 0x02)		// mouse -> relative position + mouse button
		{
			printk("sii9244_driver.c -> category 0x01 command 0x02 \n");
			action = mhlTxConfig_scratchPadData.mscScratchpadData[2];

			r_lval = 0;
			r_uval = 0;
			r_lval = (signed char) mhlTxConfig_scratchPadData.mscScratchpadData[4];
			r_uval = (signed char) mhlTxConfig_scratchPadData.mscScratchpadData[3];
			x_position = (int)((r_uval<<8) | r_lval);

			r_lval = 0;
			r_uval = 0;
			r_lval = (signed char) mhlTxConfig_scratchPadData.mscScratchpadData[6];
			r_uval = (signed char) mhlTxConfig_scratchPadData.mscScratchpadData[5];
		...
		{
			printk("sii9244_driver.c -> category 0x01 command 0x03 \n");
			action = mhlTxConfig_scratchPadData.mscScratchpadData[2];
			keyCode = ((mhlTxConfig_scratchPadData.mscScratchpadData[3] <<8) | mhlTxConfig_scratchPadData.mscScratchpadData[4]);
			...
			*/
			if(action == 0x01)
			{
				printk("sii9244_driver.c -> category 0x01 command 0x03 action 0x01\n");
				r_action = 1;

				if((kbd_key_pressed[0]>>8) == 0x00) //check
				{
			...
			{
				printk("sii9244_driver.c -> category 0x01 command 0x03 action 0x02 \n");
				r_action = 0;
	...
	{
		printk("sii9244_driver.c -> category 0x02 \n");
		command =  mhlTxConfig_scratchPadData.mscScratchpadData[1];
		action = mhlTxConfig_scratchPadData.mscScratchpadData[2];

		if(command > 0x03)
		{
			printk("sii9244_driver.c -> category 0x02 command > 0x03 \n");
			//TX_DEBUG_PRINT(("MhlControl : not defined message\n"));
			return;
		}
		...
		{
			printk("sii9244_driver.c -> category 0x02 command 0x01 \n");
			if(action == 0x01)
			{
				tmp_x = (int)((mhlTxConfig_scratchPadData.mscScratchpadData[3] <<8) | mhlTxConfig_scratchPadData.mscScratchpadData[4]);
				tmp_y = (int)((mhlTxConfig_scratchPadData.mscScratchpadData[5] <<8) | mhlTxConfig_scratchPadData.mscScratchpadData[6]);
		...
		memcpy(plugResp, mhlTxConfig_scratchPadData.mscScratchpadData, MHD_MAX_BUFFER_SIZE);
		plugResp[3] = 0x01;
		plugResp[4] = 0x00;
		plugResp[5] = 0x00;
		plugResp[6] = 0x00;
		//SiiMhlTxWriteScratchpad(plugResp);
		cbus_cmd_bind(MHL_WRITE_BURST,0x40,plugResp, MHD_MAX_BUFFER_SIZE,FALSE);
	}
	else
	{
		//TX_DEBUG_PRINT(("MhlControl : not defined message\n"));
...
static void cbus_cmd_send(struct work_struct *work)    
{
	...
	{
		//printk("sii9244_driver.c -> cbus_cmd_send CONFIG_MHL_LG_MOTION_CTL !not dev->command is MHL_WRITE_BURST \n");
		printk(" cbus_cmd_send CONFIG_MHL_LG_MOTION_CTL \n");
	#endif
		WriteByteCBUS( (REG_CBUS_PRI_ADDR_CMD&0xFF), dev->offsetdata); 	// set offset   0x13
		WriteByteCBUS( (REG_CBUS_PRI_WR_DATA_1ST & 0xFF), dev->payload_u.msgdata[0]);    //0x14
	#ifdef CONFIG_MHL_LG_MOTION_CTL
	}
	#endif
    switch(dev->command)
    {
        ...
        case MHL_MSC_MSG:
			printk("sii9244_driver.c -> cbus_cmd_send case MHL_MSC_MSG\n");
        ...
        case MHL_WRITE_BURST:
			printk("sii9244_driver.c -> cbus_cmd_send case MHK_WRITE_BURST1 \n");
			#ifdef CONFIG_MHL_LG_MOTION_CTL
...
void process_waiting_rgnd(void)
{
	...
	WriteInitialRegisterValues();	// Dec13 MHL CTS
    int4Status = I2C_ReadByte(PAGE_0_0X72, 0x74);	// read status
